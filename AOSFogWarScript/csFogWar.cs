...
namespace FischlWorks_FogWar
{
    ...
    public class csFogWar : MonoBehaviour
    {
        ...
        {
            //GameControllor.instance.existingunit
            foreach (GameObject unit in GameControllor.instance.existingunit)
            {
                if (unit != null)
                {
                    // 获取 GameObject 的 Transform
                    Transform unitTransform = unit.transform;
                    // 检查 fogRevealers 是否已包含该 transform
                    if (IsTransformInFogRevealers(unitTransform))
                    {
                        continue; // 已经存在，跳过
                    }
                    // 创建 FogRevealer 并设置属性
                    FogRevealer revealer = new FogRevealer(unitTransform, 15, false);

                    // 将 FogRevealer 添加到 fogRevealers 列表
                    fogRevealers.Add(revealer);
                }
            }
            foreach (GameObject unit in BuildingManager.instance.buildings)
            {
                if (unit != null)
                { 
                    // 获取 GameObject 的 Transform
                    ...
                    // 创建 FogRevealer 并设置属性
                    FogRevealer revealer = new FogRevealer(unitTransform, 20, false);
        ...
        bool IsTransformInFogRevealers(Transform transformToCheck)
        {
            foreach (FogRevealer revealer in fogRevealers)
            {
                if (revealer._RevealerTransform == transformToCheck)
                {
                    return true; // 已经存在
                }
            }
        ...
        private void UpdateFogField()
        {
            ...
            foreach (FogRevealer fogRevealer in fogRevealers)
            {
                ...
                else if (fogRevealer._RevealerTransform == null || fogRevealer == null)
                {
                    fogRevealers = fogRevealers.FindAll(x => x._RevealerTransform != null);
                }
            }
